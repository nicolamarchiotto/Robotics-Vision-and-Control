function [q,dq,ddq]=trap_vel_equal_acc_decc_times_given_dqc(qi,qf,ti,tf,Ts,dqc,showPlots)
...
if tc<0 || tc>((tf-ti)/2)
    ...
    error('Not fisible profile, tc>(tf-ti)/2 or tc<0 for the given constraints');
end

fprintf('trap_vel_equal_acc_decc_times_given_dqc\n');
fprintf('tc: %d\n',tc);
...
if(showPlots)
    figure;
    sgtitle('Trap Vel Profile, equal acc and decel times, dqc given')
    subplot(3,1,1)
    plot(t,q)
    xlim([ti tf])
    xlabel('time [sec]')
    ...
    grid on
    saveas(gcf,'trap_vel_equal_acc_decc_given_dqc.png')
end
end
